...
export default function App() {
  ...
  useEffect(() => {
    if (dishes.length) localStorage.setItem(STORAGE_KEY, JSON.stringify(dishes));
  }, [dishes]);

  useEffect(() => {
    if (weekMenu.length) localStorage.setItem(WEEK_KEY, JSON.stringify(weekMenu));
  }, [weekMenu]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-100 to-gray-200 p-6 max-w-md mx-auto">
      ...
      <button
        ...
        className="w-full bg-black text-white py-4 rounded-2xl mb-6 text-lg"
      >
        帮我安排一周吃什么
      </button>
